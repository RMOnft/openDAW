 ...
 *
 * Error Handling: serialization and decoding errors surface as exceptions and
 * should be caught by callers loading or saving projects.
 * @public
 */
  ...
  implements BoxAdaptersContext, Terminable, TerminableOwner
{
  ...
  static new(env: ProjectEnv): Project {
    ...
    const grooveShuffleBox = GrooveShuffleBox.create(
      boxGraph,
      UUID.generate(),
      (box) => {
    ...
    const masterBusBox = AudioBusBox.create(
      boxGraph,
      UUID.generate(),
      (box) => {
    ...
    const masterAudioUnit = AudioUnitBox.create(
      boxGraph,
      UUID.generate(),
      (box) => {
  ...
  private constructor(
    ...
    console.debug(
      `Project was created on ${this.rootBoxAdapter.created.toString()}`,
    );
  }
  ...
  get bpm(): number {
    return this.timelineBox.bpm.getValue();
  }
  /** Adapter for the project root box. */
  get rootBoxAdapter(): RootBoxAdapter {
    return this.boxAdapters.adapterFor(this.rootBox, RootBoxAdapter);
  }
  /** Adapter for the timeline box. */
  get timelineBoxAdapter(): TimelineBoxAdapter {
    return this.boxAdapters.adapterFor(this.timelineBox, TimelineBoxAdapter);
  }
