...
export namespace Communicator {
  ...
  ): PROTOCOL => bind(new Sender(messenger));
  ...
  export const executor = <PROTOCOL>(
    messenger: Messenger,
    protocol: PROTOCOL,
  ): Executor<PROTOCOL> => new Executor(messenger, protocol);
  ...
  class Sender<PROTOCOL> implements Dispatcher, Terminable {
    ...
    /** @inheritdoc */
    readonly dispatchAndReturn = <
      F extends (..._: Parameters<F>) => Promise<R>,
      R,
    >(
      func: F,
      ...args: Parameters<F>
      ...
      new Promise<R>((resolve, reject) => {
        const entries = Iterables.reduce(
          args,
          (callbacks: [int, Function][], arg: any, index: int) => {
  ...
  export class Executor<PROTOCOL> implements Terminable {
    ...
    readonly #messageHandler = (message: Send<PROTOCOL>) => {
      ...
      const object =
        Object.getPrototypeOf(this.#protocol) === Object.getPrototypeOf({})
          ? this.#protocol
      ...
      if (returnId === false) {
        ...
        try {
          const promise: Promise<any> = func.apply(
            this.#protocol,
            message.args.map((arg) =>
              "callback" in arg
                ? (...args: any[]) =>
                    this.#sendCallback(returnId, arg.callback, args)
                : arg.value,
            ),
          );
          promise.then(
            (value) => this.#sendResolve(returnId, value),
