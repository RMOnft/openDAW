/** Subscribes to MIDI message events across available inputs. */
import {
  byte,
  isDefined,
  isInstanceOf,
...
export class MIDIMessageSubscriber {
  static subscribeMessageEvents(
    ...
    const listen = (input: MIDIInput) =>
      isDefined(channel)
        ? Events.subscribe(input, "midimessage", (event: MIDIMessageEvent) => {
            if (
              event.data === null ||
              MidiData.readChannel(event.data) !== channel
            ) {
              return;
            }
            observer(event);
    ...
    const connections: Array<[MIDIInput, Subscription]> = Array.from(
