/** Access and manage MIDI devices through the Web MIDI API. */
import {
  assert,
  byte,
  isInstanceOf,
...
export class MidiDeviceAccess {
  static get(): Option<MidiDeviceAccess> {
  ...
  static available(): MutableObservableValue<boolean> {
    return new (class implements MutableObservableValue<boolean> {
        ...
        if (MidiDeviceAccess.canRequestMidiAccess()) {
          MidiDeviceAccess.#isRequesting = (() => {
            ...
            promise
              .then(
                (access) =>
                  (MidiDeviceAccess.#instance = Option.wrap(
                    new MidiDeviceAccess(access),
                  )),
              )
              .catch((reason) => {
                ...
                showInfoDialog({
                  ...
                  message: isInstanceOf(reason, Error)
                    ? reason.message
                    : String(reason),
              ...
              .finally(() => {
                ...
                AnimationFrame.once(() =>
                  MidiDeviceAccess.#notifier.notify(this),
                ); // This helps prevent Firefox from freezing
              });
          ...
          showInfoDialog({
            headline: "Cannot Access MIDI Devices",
            message:
              "This browser does not support the WebMidiApi (Hint: Chrome does).",
          }).then();
        }
      }
      ...
      getValue(): boolean {
        return (
          MidiDeviceAccess.#instance.nonEmpty() ||
          MidiDeviceAccess.#isRequesting.nonEmpty()
        );
      }
      ...
      catchupAndSubscribe(
        observer: Observer<ObservableValue<boolean>>,
      ): Subscription {
        observer(this);
  ...
  static readonly #notifier: Notifier<ObservableValue<boolean>> = new Notifier<
      ...
      none: () => {
        ...
        terminator.own(
          this.available().subscribe(() =>
            terminator.own(this.subscribeMessageEvents(observer, channel)),
          ),
        );
        return terminator;
      },
  ...
  constructor(access: MIDIAccess) {
    ...
    ConsoleCommands.exportMethod("midi.listen.all", (bool: string) => {
      subscription.terminate();
      const listen = bool === undefined ? true : Boolean(bool);
      if (listen) {
        subscription = this.subscribeMessageEvents((event) =>
  ...
  subscribeMessageEvents(
    ...
    return MIDIMessageSubscriber.subscribeMessageEvents(
      this.#access,
      observer,
      channel,
    );
