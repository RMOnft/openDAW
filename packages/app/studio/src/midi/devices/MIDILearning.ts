/** Utility for mapping MIDI controls to project parameters. */
import {
  Arrays,
  byte,
  isDefined,
...
export type MIDIConnectionJSON = (
  | { type: "key" }
  | { type: "control"; controlId: byte }
) & { address: AddressJSON; channel: byte } & JSONValue;
...
export class MIDILearning implements Terminable {
  ...
  constructor(service: StudioService) {
    ...
    this.#connections = Address.newSet<MIDIConnection>(
      (connection) => connection.address,
    );
  }

  hasMidiConnection(address: Address): boolean {
    return this.#connections.hasKey(address);
  }
  forgetMidiConnection(address: Address) {
    this.#connections.removeByKey(address).terminate();
  }
  ...
  async learnMidiKeys(engine: Engine, adapter: AudioUnitBoxAdapter) {
    ...
    learnLifecycle.own(
      MidiDeviceAccess.subscribeMessageEvents((event: MIDIMessageEvent) => {
  ...
  async learnMIDIControls(
    field: PrimitiveField<PrimitiveValues, Pointers.MidiControl | Pointers>,
  ) {
    if (!MidiDeviceAccess.canRequestMidiAccess()) {
      return;
    }
    ...
    learnLifecycle.own(
      MidiDeviceAccess.subscribeMessageEvents((event: MIDIMessageEvent) => {
        ...
        if (MidiData.isController(data)) {
          learnLifecycle.terminate();
          dialog.close();
          return this.#startListeningControl(
            field,
            MidiData.readChannel(data),
            MidiData.readParam1(data),
            event,
          );
        }
      }),
  ...
  saveToLocalStorage(key: string): void {
    localStorage.setItem(
      key,
      JSON.stringify(this.#service.midiLearning.toJSON()),
    );
  }
  ...
  loadFromLocalStorage(key: string): boolean {
    const { status, value } = tryCatch(
      () =>
        ...
        ) as ReadonlyArray<MIDIConnectionJSON>,
    );
    if (status === "failure") {
      return false;
    }
  ...
  fromJSON(json: ReadonlyArray<MIDIConnectionJSON>): void {
    ...
    this.#connections.addMany(
      json
        .map<Nullish<MIDIConnection>>((json) => {
          const { type, address: addressAsJson, channel } = json;
          const address = Address.compose(
            Uint8Array.from(addressAsJson.uuid),
            ...addressAsJson.fields,
          );
          const {
            engine,
            project: { boxGraph, boxAdapters },
          } = this.#service;
          switch (type) {
            case "key": {
              return (
                boxGraph
                  .findBox(address.uuid)
                  .ifSome((box) =>
                    this.#startListeningKeys(
                      engine,
                      boxAdapters.adapterFor(box, AudioUnitBoxAdapter),
                      channel,
                    ),
                  ) ?? undefined
              );
            ...
            case "control": {
              return (
                boxGraph.findVertex(address).ifSome((field) => {
                  if (
                    !field.isField() ||
                    !isInstanceOf(field, PrimitiveField)
                  ) {
                    return undefined;
                  }
                  return this.#startListeningControl(
  ...
  #startListeningKeys(
    ...
    const { observer, terminate } = this.#createMidiKeysObserver(
      engine,
      adapter,
    );
    const subscription = MidiDeviceAccess.subscribeMessageEvents(
      observer,
      channel,
    );
  ...
  #startListeningControl(
    ...
    console.debug(
      `startListeningControl channel: ${channel}, controlId: ${controlId}`,
    );
    const { project } = this.#service;
    const { observer, terminate } = this.#createMidiControlObserver(
      project,
      project.parameterFieldAdapters.get(field.address),
      controlId,
    );
    if (isDefined(event)) {
      observer(event);
  ...
  #createMidiKeysObserver(
    ...
    return {
      observer: (event: MIDIMessageEvent) => {
        ...
        if (
          MidiData.isNoteOff(data) ||
          (MidiData.isNoteOn(data) && MidiData.readVelocity(data) === 0)
        ) {
  ...
  #createMidiControlObserver(
    ...
    return {
      observer: (event: MIDIMessageEvent) => {
        ...
        if (
          MidiData.isController(data) &&
          MidiData.readParam1(data) === controlId
        ) {
          project.editing.modify(
            () =>
              adapter.setValue(adapter.valueMapping.y(MidiData.asValue(data))),
            false,
          );
        }
      },
