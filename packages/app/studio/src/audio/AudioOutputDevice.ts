/** Wrapper for selecting and switching audio output devices. */
import {
  asDefined,
...
export class AudioOutputDevice {
  ...
  private constructor(context: AudioContext) {
    ...
    this.#context.addEventListener("sinkchange", async () => {
      ...
      this.#value.setValue(
        status === "resolved" ? Option.wrap(value) : Option.None,
      );
      console.debug(`New Device: ${status === "resolved" ? value.label : ""}`);
    });
    this.#switchable =
      "setSinkId" in AudioContext.prototype &&
      "sinkId" in AudioContext.prototype;
    this.#value = new DefaultObservableValue<Option<MediaDeviceInfo>>(
      Option.None,
    );
  }

  get value(): DefaultObservableValue<Option<MediaDeviceInfo>> {
    return this.#value;
  }
  ...
  async setOutput(device: MediaDeviceInfo) {
    if (!this.#switchable || device.deviceId === "default") {
      return;
    }
    const { status } = await Promises.tryCatch(
      this.#context.setSinkId(device.deviceId),
    );
  ...
  async resolveOutput(): Promise<MediaDeviceInfo> {
    ...
    return AudioDevices.queryListOutputDevices().then((devices) =>
      asDefined(
        devices.find((device) => device.deviceId === sinkId),
  ...
  #resolveSinkId(): string {
    ...
    return typeof sinkId === "string"
      ? Strings.nonEmpty(sinkId, "default")
      : "default";
  }
}
