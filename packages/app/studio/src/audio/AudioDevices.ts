/**
 * High-level helpers for querying and requesting audio devices through the
...
export class AudioDevices {
  ...
  static async query<T>(query: Func<MediaDevices, T>): Promise<T> {
    if (this.#granted.isEmpty()) {
      this.#granted = Option.wrap(
        this.#request().catch((reason) => {
          console.warn(
            "Could not get permission to use microphone / audio devices.",
          );
  ...
  static async queryListInputDevices(): Promise<
    ReadonlyArray<MediaDeviceInfo>
  > {
    return this.query(this.#queryEnumerateDevices("audioinput"));
  }

  static async queryListOutputDevices(): Promise<
    ReadonlyArray<MediaDeviceInfo>
  > {
    return this.query(this.#queryEnumerateDevices("audiooutput"));
  }

  static async queryAudioInputDeviceStream(
    sampleRate: number,
    deviceId?: string,
    channelCount: int = 2,
  ): Promise<MediaStream> {
    console.debug(
      `requestAudioInputDevice deviceId: ${deviceId ?? "default"}, channelCount: ${channelCount}`,
    );
    return this.query((mediaDevices) =>
  ...
  static async #request(): Promise<void> {
    ...
    if (!(await this.#queryPermissions())) {
      return Promise.reject("Permission denied");
    }
    if (!(await this.#getUserMedia())) {
      return Promise.reject("Permission to getUserMedia denied");
    }
    console.debug("Granted permission to use microphone / audio devices.");
  }

  static #DefaultMediaTrackConstraints(
  ...
  static async #queryPermissions(): Promise<boolean> {
    ...
    if (status === "rejected") {
      return (
        (await showInfoDialog({ headline, message: String(error) })) ?? false
      );
    }
    console.debug(
      `Permission state(${permissionState.name}): ${permissionState.state}`,
    );
    ...
    if (permissionState.state === "denied") {
      return (
        (await showInfoDialog({
          headline,
          message:
            "Permissions to accept 'microphone / audio devices' has been denied.",
          buttons,
        })) ?? false
      );
    }
    if (permissionState.state === "prompt") {
      return (
        (await showInfoDialog({
          headline,
          message:
            "Your browser will now ask to request access to use your 'microphone / audio devices'.",
          buttons,
          okText: "Request",
        })) ?? true
      );
  ...
  static #queryEnumerateDevices =
    (
      kind: MediaDeviceKind,
    ): Func<MediaDevices, Promise<ReadonlyArray<MediaDeviceInfo>>> =>
    (mediaDevices) =>
      mediaDevices
        .enumerateDevices()
        .then((devices) =>
        ...
        .then((devices) =>
          devices.length > 0 ? devices : panic("Could not list devices"),
        );
}
