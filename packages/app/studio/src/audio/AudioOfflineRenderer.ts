/**
 * Utilities for rendering projects offline using the browser's
 * `OfflineAudioContext`.
 */
...
import {
  ...
  showProcessDialog,
  showProcessMonolog,
} from "@/ui/components/dialogs.tsx";
...
export namespace AudioOfflineRenderer {
  ...
  export const start = async (
    ...
    const numStems = ExportStemsConfiguration.countStems(
      optExportConfiguration,
    );
    ...
    const numSamples = PPQN.pulsesToSamples(
      durationInPulses,
      project.bpm,
      sampleRate,
    );
    ...
    const engineWorklet = worklets.createEngine(
      project,
      optExportConfiguration.unwrapOrUndefined(),
    );
    engineWorklet.play();
    ...
    const terminable = AnimationFrame.add(() =>
      progress.setValue(context.currentTime / durationInSeconds),
    );
    ...
    if (optExportConfiguration.isEmpty()) {
      ...
      await saveZipFile(
        buffer,
        meta,
        Object.values(optExportConfiguration.unwrap()).map(
          ({ fileName }) => fileName,
        ),
      );
    }
  ...
  const saveWavFile = async (buffer: AudioBuffer, meta: ProjectMeta) => {
    ...
    const saveResult = await Promises.tryCatch(
      Files.save(wavFile, { suggestedName }),
    );
  ...
  const saveZipFile = async (
    ...
    const saveResult = await Promises.tryCatch(
      Files.save(arrayBuffer, { suggestedName: `${meta.name}.zip` }),
    );
