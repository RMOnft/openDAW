...
// Base class name applied to the resizer element.
...
/**
 * Draggable divider used to resize two adjacent flex panels. Adjusts the
 * `flex` values of the neighbouring panels while respecting their min/max
 * size constraints.
 */
export const PanelResizer = ({
  ...
  const element: HTMLElement = (
    <div className={Html.buildClassList(className, orientation)} />
  );
  const direction = AxisProperty[orientation];
  lifecycle.own(
    Dragging.attach(element, (begingEvent) => {
      ...
      const elements = siblings.map(({ content, element }) => {
        return {
          content: content,
          element: element,
      ...
      if (!isDefined(curr) || !isDefined(next)) {
        return Option.None;
      }
      if (
        panelContents.isClosed(curr.content) ||
        panelContents.isClosed(next.content)
      ) {
        return Option.None;
      }
      ...
      return Option.wrap({
        update: (event: Dragging.Event): void => {
          ...
          const sizeA = clamp(
            beginSizeA + delta,
            Math.max(minSizeA, sumSize - maxSizeB),
            Math.min(maxSizeA, sumSize - minSizeB),
          );
          const sizeB = clamp(
            beginSizeB - delta,
            Math.max(minSizeB, sumSize - maxSizeA),
            Math.min(maxSizeB, sumSize - minSizeA),
          );
