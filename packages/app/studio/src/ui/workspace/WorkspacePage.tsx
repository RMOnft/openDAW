  ...
  const build = (
    ...
    const element: HTMLElement = (() => {
      if (content.type === "panel") {
        ...
        const section = (
          <section
            className={Html.buildClassList("workspace", content.orientation)}
          >
            <div className="fill" />
          </section>
        );
...
export const WorkspacePage: PageFactory<StudioService> = ({
  ...
  const mainElement: HTMLElement = <main aria-label="Workspace" />;
  const screenLifeTime = lifecycle.own(new Terminator());
  lifecycle.own(
    service.layout.screen.catchupAndSubscribe((owner) => {
