...
/**
 * Represents the configuration and runtime state of a single panel within the
 * workspace. Instances are created from {@link Workspace.PanelConfig} and
 * track whether the panel is minimized or pop-out capable.
 */
export class PanelState {
  static create(schema: Workspace.PanelConfig): PanelState {
    return new PanelState(schema);
  }
  ...
  get constrains(): Workspace.LayoutConstrains {
    return this.#constrains;
  }
  get isMinimized(): boolean {
    return this.#minimized && this.#minimizable;
  }
  set isMinimized(value: boolean) {
    this.#minimized = value && this.#minimizable;
  }
  get minimizable(): boolean {
    return this.#minimizable;
  }
  get popoutable(): boolean {
    return this.#popoutable;
  }
}
