...
// Base CSS class for all panel placeholders.
...
/**
 * Renders a panel container with a header and optional context menu. It hosts
 * the actual panel content and keeps its flex sizing in sync with sibling
 * panels.
 */
export const PanelPlaceholder = ({
  ...
  const element: DomElement = (
    <div
      className={Html.buildClassList(className, orientation)}
  ...
  // Adjust the element and sibling layout whenever the panel state changes.
  const updateLayout = () => {
    ...
    if ((panelState.isMinimized || panelContent.isPopout) && minimizable) {
      ...
      if (constrains.type === "fixed") {
        style.flexGrow = "0";
        style[property.minStyle] = style[property.maxStyle] =
          `${constrains.fixedSize + HeaderSize}px`;
      } else if (constrains.type === "flex") {
        style.flexGrow = constrains.flex.toString();
        style[property.minStyle] = `${constrains.minSize}px`;
        style[property.maxStyle] =
          `${constrains.maxSize ?? Number.MAX_SAFE_INTEGER}px`;
      }
    }

    // Update siblings flexGrow
    const flexible: Array<{
      constrains: FlexLayoutConstrains;
      style: CSSStyleDeclaration;
    }> = [];
    siblings.forEach(({ content, element: { style } }) => {
      if (
        !panelContents.isClosed(content) &&
        content.constrains.type === "flex"
      ) {
  ...
  const popupLabel: Element = (
    <div className="popup-label">
      <span>{`${panelState.name} is currently in another window.`}</span>
    </div>
  );
  ...
  const handler = lifecycle.own(
    panelContent.bind(panelState, container, {
      onEmbed: () => {
  ...
  lifecycle.own(
    Events.subscribe(header, "dblclick", () => handler.toggleMinimize()),
  );
  lifecycle.own(
