...
export namespace ContextMenu {
  ...
  export const install = (owner: WindowProxy): Subscription => {
    return Events.subscribe(
      ...
      async (mouseEvent: MouseEvent) => {
        ...
        const event: Event = new Event(CONTEXT_MENU_EVENT_TYPE, {
          bubbles: true,
          composed: true,
          cancelable: true,
        });
        ...
        if (CollectorImpl.collecting.nonEmpty()) {
          ...
          if (collector.hasItems) {
            ...
            menu.element.setAttribute("aria-label", "Context menu");
            menu.moveTo(x, y);
  ...
  export const subscribe = (
    ...
    Events.subscribeAny(
      target,
      CONTEXT_MENU_EVENT_TYPE,
      (event) => {
        CollectorImpl.collecting.match({
          none: () =>
            panic(
              `Got collect event ${event} without being in populating phase`,
            ),
