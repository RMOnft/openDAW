...
/** Test data used to render individual automation scenarios. */
type EdgeCase = {
  label: string;
...
/** Collection of edge case configurations to visualise. */
...
/**
 * Renders a development page showcasing automation rendering edge cases.
 */
export const AutomationPage: PageFactory<
  StudioService
      ...
      <div>
        ...
        <div>
          {EdgeCases.map(({ label, section, events }) => {
            ...
            const valueToY = (x: unitValue): number =>
              actualHeight - radius - x * (actualHeight - radius * 2);
            const eventCollection = EventCollection.create(
              ValueEvent.Comparator,
            );
            ...
            for (const pass of LoopableRegion.locateLoops(
              ...
              if (pass.index > 0) {
                const x = Math.floor(
                  range.unitToX(pass.resultStart) * devicePixelRatio,
                );
              ...
              const iterator = ValueEvent.iterateWindow(
                eventCollection,
                windowMin,
                windowMax,
              );
            ...
            for (const event of ValueEvent.iterateWindow(
              eventCollection,
              offset,
              offset + section.loopDuration,
            )) {
              const x =
                range.unitToX(offset + section.loopDuration + event.position) *
                devicePixelRatio;
            ...
            for (let i = 1; i < N; i++) {
              ...
              const x =
                range.unitToX(offset + section.loopDuration + position) *
                devicePixelRatio;
              context.moveTo(x, 0);
            ...
            const iteratable = ValueEvent.quantise(
              eventCollection,
              0,
              section.loopDuration,
              N,
            );
