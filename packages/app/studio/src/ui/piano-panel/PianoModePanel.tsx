/**
 * Hosts the piano roll visualizers and controls for configuring Piano Mode. It
 * combines {@link PianoRoll} and {@link NoteFall} with various settings such as
 * keyboard size, note width and transpose.
 */
...
/** Props for {@link PianoModePanel}. */
type Construct = {
  /** Owner responsible for lifecycle management. */
  lifecycle: Lifecycle;
  /** Provides access to the current studio project and engine. */
  service: StudioService;
};

/**
 * Entry point for the piano mode UI. Displays controls and subscribes to
 * project changes to update the visualizers.
 */
export const PianoModePanel = ({ lifecycle, service }: Construct) => {
  ...
  const subscribeExcludePianoModeAll = (
  ...
  const subscribeExcludePianoMode = () => {
    excludePianoModeSubscription = subscribeExcludePianoModeAll(
      rootBoxAdapter,
      () => {
    ...
    <div className={className}>
      ...
      <div className="controls">
        <Group>
          ...
          <NumberInput
            lifecycle={lifecycle}
            model={EditWrapper.forValue(editing, noteScale)}
            step={0.1}
          ...
          <NumberInput
            lifecycle={lifecycle}
            model={EditWrapper.forValue(editing, transpose)}
            step={1}
          ...
          {tracksHeader}
          {rootBoxAdapter.audioUnits.adapters().flatMap((audioUnitBoxAdapter) =>
            audioUnitBoxAdapter.tracks
              .values()
                  ...
                  <span>
                    {
                      ...
                      array.length === 1
                        ...
                        : `${audioUnitBoxAdapter.label} (${index + 1})`
                    }
                  </span>
                  <Checkbox
                    lifecycle={lifecycle}
                    model={EditWrapper.forValue(
                      editing,
                      MutableObservableValue.inverseBoolean(
                        track.box.excludePianoMode,
                      ),
                    )}
                  >
                    <Icon symbol={IconSymbol.Checkbox} />
