/**
 * Canvas based visualization that draws falling notes for the Piano Mode. Notes
 * scroll from top to bottom representing their temporal position relative to
 * the playback cursor.
 */
...
/** Props for {@link NoteFall}. */
type Construct = {
  /** Lifecycle owner for automatic cleanup. */
  lifecycle: Lifecycle;
  /** Studio service providing project context. */
  service: StudioService;
  /** Notifier that triggers redraws. */
  updateNotifier: Notifier<void>;
};

/** Describes a single note to be rendered on the canvas. */
type RenderCall = {
  pitch: int;
  x: number;
  y0: number;
...
/**
 * Renders falling notes on a canvas synchronized with the current engine
 * position.
 */
export const NoteFall = ({ lifecycle, service, updateNotifier }: Construct) => {
  ...
  const { keyboard, timeRangeInQuarters, noteScale, noteLabels, transpose } =
    pianoMode;
  ...
  const painter = new CanvasPainter(canvas, (painter) => {
    ...
    const positionToY = (position: ppqn) =>
      (1.0 - (position - min) / timeRange) * actualHeight;
    ...
    const stepSize = PPQN.fromSignature(
      nominator.getValue(),
    ...
    project.rootBoxAdapter.audioUnits.adapters().forEach((audioUnitAdapter) => {
      const trackBoxAdapters = audioUnitAdapter.tracks
        .values()
        .filter((adapter) => !adapter.box.excludePianoMode.getValue());
      trackBoxAdapters.forEach((trackAdapter) => {
        for (const region of trackAdapter.regions.collection.iterateRange(
          min,
          max,
        )) {
          if (!isInstanceOf(region, NoteRegionBoxAdapter)) {
            continue;
          }
          ...
          for (const {
            ...
            resultEnd,
            rawStart,
          } of LoopableRegion.locateLoops(region, min, max)) {
            const searchStart = Math.floor(resultStart - rawStart);
            const searchEnd = Math.floor(resultEnd - rawStart);
            for (const note of events.iterateRange(
              searchStart - collection.maxDuration,
              searchEnd,
            )) {
        ...
        MidiKeys.Names.English[pitch % 12]
          ...
          .forEach((letter, index) =>
            context.fillText(
              letter,
              x,
              y1 - index * noteWidth * 0.45 * devicePixelRatio,
            ),
          );
        context.restore();
      });
  ...
  lifecycle.ownAll(
    ...
    Events.subscribe(canvas, "wheel", (event) => {
      ...
      const position =
        enginePosition.getValue() -
        Math.sign(event.deltaY) * PPQN.SemiQuaver * 2;
