/**
 * Visualizes a static piano keyboard within the Piano Mode panel and highlights
 * notes that are currently playing. The component observes the active project
 * and engine position and updates key states accordingly.
 */
...
/** Props for {@link PianoRoll}. */
type Construct = {
  /** Owner responsible for lifecycle management. */
  lifecycle: Lifecycle;
  /** Access to project and audio engine services. */
  service: StudioService;
  /** Notifier used to trigger visual updates. */
  updateNotifier: Notifier<void>;
};

/**
 * Renders the piano keyboard SVG and updates key fill styles to reflect
  ...
  const update = (position: ppqn) => {
    ...
    project.rootBoxAdapter.audioUnits.adapters().forEach((audioUnitAdapter) => {
      ...
      trackBoxAdapters.forEach((trackAdapter) => {
        ...
        if (
          region === null ||
          !isInstanceOf(region, NoteRegionBoxAdapter) ||
        ...
        const loopIterator = LoopableRegion.locateLoops(
          region,
          position,
          position,
        );
        for (const { resultStart, resultEnd, rawStart } of loopIterator) {
          ...
          for (const note of events.iterateRange(
            ...
            if (
              note.position + rawStart <= position &&
              position < note.complete + rawStart
            ) {
