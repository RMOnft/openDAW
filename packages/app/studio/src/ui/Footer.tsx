...
export const Footer = ({ lifecycle, service }: Construct) => {
  ...
  const updateOnline = () =>
    (labelOnline.textContent = navigator.onLine ? "Yes" : "No");
  lifecycle.ownAll(
  ...
  const labelName: HTMLElement = (
    <div
      ...
      ondblclick={(event) => {
        ...
        if (isDefined(name)) {
          Surface.get(labelName)
            .requestFloatingTextInput(event, name)
  ...
  lifecycle.own(
    service.sessionService.catchupAndSubscribe((owner) => {
      ...
      if (optSession.nonEmpty()) {
        const session = optSession.unwrap();
        const observer = (meta: ProjectMeta) =>
  ...
  const lastBuildTime = TimeSpan.millis(
