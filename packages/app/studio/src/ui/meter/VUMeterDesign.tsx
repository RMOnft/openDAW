...
/** Predefined VU meter visual designs. */
export namespace VUMeterDesign {
  /** Classic analog style VU meter design. */
  export const Default = ({ model }: { model: ObservableValue<number> }) => {
    ...
    const needle: SVGLineElement = (
      <line
        x1={0}
        x2={0}
        y1={0}
        y2={-geometry.needleRadius - 28}
        stroke="hsl(36,25%,30%)"
        stroke-width={2}
      />
    );
    const background: SVGGraphicsElement = (
      ...
      >
        {geometry
          .buildStripe({ fill: color.black })
          .setSection(0.0, geometry.unitToU(0.0), 0.0, 8.0)
          .addMarker(
            geometry.unitToU(Number.NEGATIVE_INFINITY),
            geometry.unitToU(-24),
            4,
          )
          .addMarkerAt(-20, 4, 16)
        ...
        {geometry
          ...
          .addMarkerAt(gainToDb(1.0), 2, -4)
          .addMarkerAt(gainToDb(0.75), 2, -4)
          .addMarkerAt(gainToDb(0.5), 2, -4)
          .addMarkerAt(gainToDb(0.25), 2, -4)
          .addMarkerAt(gainToDb(0.0), 2, -4)
          .build()}
        {geometry
          .buildStripe({ fill: color.red })
          .setSection(
            geometry.unitToU(0.0, 6.0),
            geometry.unitToU(3.0),
            0.0,
            12.0,
          )
          .addMarkerAt(1, 2, 6)
          ...
          .addMarkerAt(3, 4, 12)
          .build()}
        {...[-20, -10, -7, -5, -3, 0, 3].map((db) =>
          geometry.buildLabel(
            geometry.unitToU(db),
            Math.abs(db).toString(),
            32,
            12,
            geometry.degAt(geometry.unitToU(db)),
            { fill: db > 0 ? color.red : color.black },
          ),
        )}
        {...[0, 25, 50, 75, 100].map((percentage) =>
          geometry.buildLabel(
            geometry.unitToU(gainToDb(percentage / 100.0)),
            `${percentage}%`,
            -22,
            7,
            0.0,
  ...
  /** Modern style VU meter design with extended range. */
  export const Modern = ({ model }: { model: ObservableValue<number> }) => {
    const geometry = new VUMeter.Geometry(
      new GainMapping(6, 1.5),
      144,
      256,
      24,
    );
      ...
      >
        ...
        {...[-20, -10, -7, -5, -3, -2, -1, 0, 3].map((db) =>
          geometry.buildLabel(
            ...
            12,
            0.0,
            { fill: db >= 0 ? color.red : color.yellow },
          ),
        )}
        {...[0, 30, 50, 70, 100].map((percentage) =>
          geometry.buildLabel(
            geometry.unitToU(gainToDb(percentage / 100.0)),
