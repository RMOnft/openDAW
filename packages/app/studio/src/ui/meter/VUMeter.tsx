...
/** Utilities and components for rendering analog style VU meters. */
export namespace VUMeter {
  /**
   * Visual design configuration for a {@link Element} instance including size,
   * geometry and SVG elements.
   */
  ...
  /**
   * React component rendering the VU meter SVG using the provided design and
  ...
  export const Element = ({
    ...
    const updateNeedle = (gain: number) =>
      needle.setAttribute(
        "transform",
        `translate(${anchor.x}, ${
          anchor.y + geometry.needleRadius
    ...
    return (
      ...
      >
        ...
        {background}
        <g style={{ filter: "drop-shadow(0px 8px 2px rgba(0, 0, 0, 0.5))" }}>
          {needle}
        </g>
      </svg>
    );
  ...
  /** Geometry helper to transform values into meter coordinates. */
  export class Geometry {
    ...
    get scaleRadian(): number {
      return this.#scaleRadian;
    }
    ...
    unitToU(unit: number, offset: number = 0.0): number {
      return this.positionToU(this.#mapping.x(unit), offset);
    }

    unitToNeedleDegree(unit: number): number {
      const p = Point.subtract(
        this.localToGlobal(clamp(this.#mapping.x(unit), 0.0, 1.0), 0.0),
        this.#needleAnchor,
      );
    ...
    localToGlobal(u: number, v: number): Point {
      ...
      return v === 0.0
        ? p
        : Point.add(p, Point.scaleTo(Point.subtract(p, this.#needleAnchor), v));
    }

    radianAt(x: number): number {
      return x * this.#scaleRadian * 2.0 - this.#scaleRadian;
    }

    degAt(x: number): number {
      return (this.radianAt(x) * 180.0) / Math.PI;
    }

    buildStripe(style: PaintStyle): StripeBuilder {
      return new StripeBuilder(this, style);
    }
    ...
    buildLabel(
      ...
      size: number,
      angle: number,
      { fill, stroke }: PaintStyle = {},
    ): SVGTextElement {
      const p0 = this.localToGlobal(u, 0.0);
      const p1 = Point.add(
        p0,
        Point.scaleTo(Point.subtract(p0, this.#needleAnchor), distance),
      );
      return (
        <text
          font-size={`${size}px`}
  ...
  /** Builder for drawing radial stripes and markers on the meter scale. */
  class StripeBuilder {
    ...
    addMarkerAt(
      ...
      if (align === "start") {
        this.addMarker(
          this.#geometry.unitToU(unit, 0.0),
          this.#geometry.unitToU(unit, width),
          length,
        );
      } else if (align === "center") {
        this.addMarker(
          this.#geometry.unitToU(unit, -width / 2.0),
          this.#geometry.unitToU(unit, width / 2.0),
          length,
        );
      } else if (align === "end") {
    ...
    build(): SVGPathElement {
      ...
      const u0 =
        markers?.reduce((x, marker) => Math.min(x, marker.u0), this.#u0) ??
        this.#u0;
      const u1 =
        markers?.reduce((x, marker) => Math.max(x, marker.u1), this.#u1) ??
        this.#u1;
      ...
      for (const marker of markers
        .filter((marker) => marker.length > 0.0)
        .sort((a, b) => a.u0 - b.u0)) {
