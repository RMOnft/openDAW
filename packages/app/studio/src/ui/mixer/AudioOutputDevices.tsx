...
export const AudioOutputDevices = ({ output, provider }: Construct) => {
  return (
    <div className={className} style={{ padding: "1em 0" }}>
      <Await
        factory={() => AudioDevices.queryListOutputDevices()}
        loading={() => <div>Please allow access to list audio-devices...</div>}
        ...
        success={(devices) => {
          ...
          output.resolveOutput().then(
            (output) =>
              list
                .querySelector(`div[deviceId="${output.deviceId}"]`)
